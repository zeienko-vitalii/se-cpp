...
void CALLBACK GraphScreen::onTimerAction(PTP_CALLBACK_INSTANCE instance, PVOID param,
		PTP_TIMER timer) {
	TimerParam *par;
	...
	if (par->numCntActions-- > 0) {
		...
		char l = 186, r = 186, c = 179;
		cout << l << setw(12) << left << "height" << c << setw(12)
				<< par->paramRoom->getHeight() << r << endl;
		cout << l << setw(12) << left << "width" << c << setw(12)
				<< par->paramRoom->getWidth() << r << endl;
		cout << l << setw(12) << left << "length" << c << setw(12)
				<< par->paramRoom->getLength() << r << endl;

		char ld = 200, rd = 188, dh = 207;
		cout << ld;
		for (int i = 0; i < 12; i++, cout << h)
			;
		cout << dh;
