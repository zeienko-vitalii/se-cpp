...
#include <iostream>
namespace Screen {
	GraphScreen::GraphScreen(Wheel::Wheel * _wheel) :wheel(new Wheel::Wheel(_wheel)) {}
	GraphScreen::~GraphScreen() {
		if (wheel)delete wheel;
	}
	void GraphScreen::SetDataSource(const Wheel::Wheel &_wheel) {
		if (wheel)delete wheel;
		wheel = new Wheel::Wheel(_wheel);
	}

	Wheel::Wheel GraphScreen::GetDataSource()const { return new Wheel::Wheel(wheel); }
